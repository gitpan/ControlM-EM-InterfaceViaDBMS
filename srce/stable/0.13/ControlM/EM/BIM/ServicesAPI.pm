...
Carp, Hash::Util, Exporter, Time::Local, POSIX, DBI, /^DBD::(Pg|mysql|Oracle|Sybase|ODBC)$/
...
Retourne la valeur de la propriete "$propertyName", et leve une exception si celle-ci n existe pas.

=item - $obj->I<setPublicProperty($propertyName, $value)>

Remplace la valeur de la propriete publique "$propertyName" par "$value".

Retourne 1 si la valeur de la propriete ete modifiee, 0 si c est une propriete privee et leve une exception si celle-ci n existe pas.
    ...
    *new = \&newSession;
    ...
    sub getCurrentServices {
        ...
        if ($self->getSessionIsConnected()) {
            ($situation, $datacenterInfos) = $_getDatasCentersInfos->($self->{'_DBI'});
            if ($situation) {
                my $time = time;
                    ...
                    if ($situation) {
                        ...
                        unless ($downloadTimeInTimestamp == 0 || $@) {
                            if ($downloadTimeInTimestamp >= $datacenterOdateStart && $downloadTimeInTimestamp <= $datacenterOdateEnd) {
                                ($situation, $jobsInformations{$dataCenter}) = $_getBIMJobsFromActiveNetTable->($self->{'_DBI'}, $params{'handleDeletedJobs'}, $datacenterInfos->{$dataCenter}->{'active_net_table_name'});
                ...
                ($situation, my $servicesDatas) = $_getAllServices->($self->{'_DBI'}, $params{'matching'}, \%jobsInformations, $datacenterInfos, $params{'forLastNetName'});
    ...
    sub getProperty {
        my ($self, $property) = @_;
        Carp::croak($_myErrorMessage->((caller 0)[3], "propiete inexistante '" . $property . "'.")) unless (exists $self->{$property});
    ...
    sub setPublicProperty {
        my ($self, $property, $value) = @_;
        if ($property =~ /^_/) {
            Carp::croak($_myErrorMessage->((caller 0)[3], "tentative de modication d'une propriete privee ('" . $property . "')."));
        } elsif (exists $self->{$property}) {
            $_setObjProperty->($self, $property, $value);
            return 1;
        }
        return 0;
    }
    ...
    sub clearError {
        my $self = shift;
        $_setObjProperty->($self, '_errorMessage', undef);
        return 1;
    }
    ...
    sub getSessionIsAlive {
        ...
        if ($self->{'_DBI'} && $self->getSessionIsConnected()) {
            ...
            $self->{'_errorMessage'} = $_myErrorMessage->((caller 0)[3], "impossible de tester l'etat de la connexion au SGBD car celle ci n'est pas active.");
            return 0;
        }
    }
    ...
    sub getSessionIsConnected {
        my $self = shift;
        return $self->{'_sessionIsConnected'};
    }
...

{
    ...
    my $_getAllViaLogID = sub {
        ...
        my $sth = $dbh->prepare($sqlRequest . " WHERE log_id IN ('" . join("', '", @servicesLogID) . "');");
        if ($sth->execute()) {
...
XML
        ...
        if ($self->{'_ControlM::EM::BIM::ServicesAPI'}->getSessionIsConnected()) {
            if ($self->{'_currentServices'}) {
                if (my @servicesLogID = keys %{$self->{'_currentServices'}}) {
                    my ($situation, $hashRefPAlertsJobsForServices) = $_getAllViaLogID->($self->{'_ControlM::EM::BIM::ServicesAPI'}->{'_DBI'}, 'SELECT * FROM bim_alert', @servicesLogID);
                    if ($situation) {
    ...
    sub getProblematicsJobsByServices {
        ...
        if ($self->{'_ControlM::EM::BIM::ServicesAPI'}->getSessionIsConnected()) {
            if ($self->{'_currentServices'}) {
                if (my @servicesLogID = keys %{$self->{'_currentServices'}}) {
                    my ($situation, $hashRefProblematicsJobsForServices) = $_getAllViaLogID->($self->{'_ControlM::EM::BIM::ServicesAPI'}->{'_DBI'}, 'SELECT * FROM bim_prob_jobs', @servicesLogID);
                    if ($situation) {
